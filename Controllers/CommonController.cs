...
namespace astute.Controllers
{
    ...
    public class CommonController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("getcountries")]
        public async Task<IActionResult> GetCountries([FromForm] int countryId, [FromForm] string countryName, [FromForm] string isdCode, [FromForm] string shortCode)
        {
            try
            {
                var result = await _commonService.GetCountries(countryId, countryName, isdCode, shortCode);
        ...
        [HttpGet]
        [Route("getstates")]
        public async Task<IActionResult> GetStates(int stateId, string state, int countryId, string stdCode)
        {
            try
            {
                var result = await _commonService.GetStates(stateId, state, countryId, stdCode);
                if (result != null && result.Count > 0)
                {
                    return Ok(new
                    {
