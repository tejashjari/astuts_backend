...
namespace astute.Repository
{
    public partial class CategoryService : ICategoryService
    {
        ...
        public async Task<Category_Master> GetCategoryById(int catId)
        {
            var param = new SqlParameter("@cat_Id", catId);

            var category = await Task.Run(() => _dbContext.Category_Master
                            .FromSqlRaw(@"exec GetCategoryByCat_Id @cat_Id", param).ToListAsync());

            return category.FirstOrDefault();
        }

        public async Task<IList<Category_Master>> GetCategoriesByColId(int colId)
        {
            var param = new SqlParameter("@colId", colId);

            var categories = await Task.Run(() => _dbContext.Category_Master
                            .FromSqlRaw(@"exec GetCategoryByColID @colId", param).ToListAsync());
