...
namespace astute.Repository
{
    public partial class CompanyService : ICompanyService
    {
            ...
            IConfiguration configuration)
        {
            ...
            _connectionString = configuration.GetConnectionString("AstuteConnection");
        }
        #endregion
        ...
        public async Task<Company_Master> GetCompanyById(int companyId)
        {
            var param = new SqlParameter("@companyId", companyId);

            var result = await Task.Run(() => _dbContext.Company_Master
                            .FromSqlRaw(@"exec Get_Company_Master_By_CompanyId @companyId", param).ToListAsync());

            return result.FirstOrDefault();
        }

        public async Task<IList<Company_Master>> GetAllCompanies()
        {
            var result = await Task.Run(() => _dbContext.Company_Master
                            .FromSqlRaw(@"exec Company_Master_Select").ToListAsync());
